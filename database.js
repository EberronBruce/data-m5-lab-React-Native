...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    ...
    const insertStatement = `INSERT INTO menuitems (uuid, title, price, category) VALUES ${values};`;

    tx.executeSql(insertStatement, [], (_, { rowsAffected }) => {
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    db.transaction(
			(tx) => {
				const categoryFilter = activeCategories.map(
					(category) => `category = '${category}'`
				);
				const categoryFilterString = categoryFilter.join(" OR ");

				const queryString = `
        SELECT * FROM menuitems
        WHERE title LIKE '%${query}%'
        ${categoryFilter.length > 0 ? `AND (${categoryFilterString})` : ""}
      `;
